...
from Get_Data import aggregate_data_by_season, update_df, get_player_id
...
'Blocked Shots', 'Steals', 'Turnovers', 'Rebs+Asts', 'Blks+Stls', 'Pts+Rebs+Asts', 'Pts+Rebs', 'Pts+Asts', 'Points', 'Rebounds','Assists']
    ...
    with open(f'PrizePicksOdds/prizepicks_{date}.json', 'r') as file:
...
def load_display_future_data(date, category, num_picks, sampling_type='random', odds_type='standard',display_only=False):
    num_picks_all = [3, 4, 5, 6, num_picks]
    if not display_only:
        update_df()
...
def aggregate_preds_across_intervals(dates):
    ...
    for cat in cat_bounds.keys():
        ...
        best_accuracy = 0
        threshold_acc_map = {}
        for threshold in np.arange(cat_bounds[cat][0], cat_bounds[cat][1], 0.5):
            each_day_array = []
            cor_sum = 0
            total_sum = 0
            for date in dates:
                    ...
                    if cat in df['Category'].unique():
                        accuracy, cor, total, _ = evaluate_prize_picks(date, cat, threshold=threshold)
...
def accuracy_across_days(dates, selection_type, num_picks=6, category='all', odds_type='standard', show_acc=False):
    total_correct = 0
    total_count = 0
    acc_by_day = []
    for date in tqdm(dates):
